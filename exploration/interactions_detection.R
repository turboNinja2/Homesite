...
train <- train[sample(nrow(train),size = 100000),]
print(dim(train))
...
{
  if(name1!="QuoteConversion_Flag")
  {
    if(length(table(train[,name1]))>13){next}
	for(name2 in var_names)
	{
    if(name1==name2) {break}
    if(name2!="QuoteConversion_Flag")
	  {
	    str_formula_interaction <- paste0("QuoteConversion_Flag~",name1,"*",name2)
	    str_formula_no_interaction <- paste0("QuoteConversion_Flag~",name1,"+",name2)
